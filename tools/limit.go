...
type LimitQueueMap struct {
	sync.RWMutex
	LimitQueue map[string][]int64
}

func (l *LimitQueueMap) readMap(key string) ([]int64, bool) {
	l.RLock()
...
func (l *LimitQueueMap) writeMap(key string, value []int64) {
	l.Lock()
	l.LimitQueue[key] = value
	l.Unlock()
}

var LimitQueue = &LimitQueueMap{
